...
from utils.label_prop_visualization import label_prop_visualization
...
def main():
    ...
    parser.add_argument('--out', type=str, default="mgn_train", help='Name output directory.')
    ...
    parser.add_argument('--num_clusters', type=int, nargs="*", default=[10, 5], help="Select learned resolutions (default is [10, 5])")
    parser.add_argument('--optimal_clusters_felz', action='store_true', help='Select optimal clusters with felzenswalb segmentation')
    parser.add_argument('--felz_threshold', type=float, default=0.8, help='Select threshold for optimal felz segmentation')
    parser.add_argument('--optimal_clusters_kmeans', action='store_true', help='Select optimal clusters with kmeans clustering')
    parser.add_argument('--kmeans_threshold', type=int, default=25, help='Select threshold for optimal kmeans clustering')
    ...
    data = construct_feature_graph(segments, dataset, ground_truth, # Feature Extraction Pipeline
                               args.train_size, args.seed, args.beta, args.sigma_s, args.knn_k, args.k, args.verbal, out)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # call model, optimizer, loss function
    model = MGNN(nfeat = dataset.shape[2],
                nhid = args.nhid,
                nout = len(np.unique(ground_truth[ground_truth!=0])),
                n_nodes = len(np.unique(segments)),
                dropout=args.dropout, num_clusters = args.num_clusters,
    ...
    for epoch in tqdm(range(args.epochs+1)): # train, test loop (in: graph of each band: out: loss, acc)
        loss = train(model, device, optimizer, criterion, data)
        acc, _, _, _ = test(model, device, segments, ground_truth, data, verbal=(epoch%50==0), out=val_img_dir, figure="epoch_{}".format(epoch))
    ...
    print("Test results:")
    print("==============================")
    test(model, device, segments, ground_truth, data, verbal=True, out=out, figure="output.png")
