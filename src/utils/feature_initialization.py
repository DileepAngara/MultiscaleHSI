...
def generate_features(segments, img, alpha=1):
    ...
    for label in unique_labels:
        ...
        neighbor_weights = [
            (calc_weight(mean_vector, neighbor_vector, alpha))
        ...
        if np.sum(neighbor_weights) > 0:
            weighted_features[label] = np.sum(
                mean_features[neighbor_labels] * neighbor_weights[..., np.newaxis],
                axis=0,
            )
        else:
            weighted_features[label] = mean_vector

    return (
        torch.tensor(mean_features, dtype=torch.float),
        torch.tensor(weighted_features, dtype=torch.float),
        torch.tensor(np.array(centroids), dtype=torch.float),
    )
...
def generate_weights(
    mean_features, weighted_features, centroids, edge_index, beta=0.5, sigma_s=1
):
    edge_attr = []
    for i, j in zip(edge_index[0], edge_index[1]):
        ...
        sij = np.exp(
            ((beta - 1) * mean_vec_norm**2 - beta * weighted_vec_norm**2) / sigma_s**2
        )
