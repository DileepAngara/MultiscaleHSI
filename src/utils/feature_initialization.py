...
def calc_weight(vector1, vector2, alpha):
...
def generate_features(segments, img, alpha=1):
  ...
  for label in unique_labels:
    ...
    if np.sum(neighbor_weights) > 0:
      ...
      weighted_features[label] = mean_features[label] # Weights arent strong enough then consider it as its own neighborhood
...
def train_test_mask(matrix_shape, test_size, random_state):
  np.random.seed(random_state)

  # Calculate the total number of elements
  total_elements = matrix_shape[0] * matrix_shape[1]

  # Calculate the number of elements for testing
  num_test_elements = int(test_size * total_elements)

  # Create a mask matrix with random assignment of elements to training and testing sets
  mask = np.ones(total_elements, dtype=bool)
  mask[:num_test_elements] = False  # Set the first num_test_elements elements to True (for testing)
  np.random.shuffle(mask)  # Shuffle the mask to randomize the assignment

  # Reshape the mask into the original matrix shape
  mask_matrix = mask.reshape(matrix_shape)

  # Check the shape of the mask matrix
  print("Shape of mask matrix:", mask_matrix.shape)
...
def generate_weights(mean_features, weighted_features, centroids, edge_index, beta = 0.5, sigma_s = 1, sigma_l = 0.5):
  edge_attr = []
  for i, j in zip(edge_index[0], edge_index[1]):
    ...
    lij = np.exp((-centroid_vec_norm**2)/sigma_l**2)
