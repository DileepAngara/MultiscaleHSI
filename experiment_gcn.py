...
def main():
    ...
    logging.info(f"Processing model: GCN")
    ...
    model = GCN(in_channels=NFEAT, hidden_channels=NHID, out_channels=NOUT, num_layers=2, norm="layernorm").to(device)
    ...
    if args.training:
        ...
        logging.info(f'Saving model to {os.path.join(EXPERIMENT_RESULT_PATH, "gcn_model.pth")}')
        torch.save(model.state_dict(), os.path.join(EXPERIMENT_RESULT_PATH, "gcn_model.pth"))
        torch.save(optimizer.state_dict(), os.path.join(EXPERIMENT_RESULT_PATH, "gcn_optimizer.pth"))
        np.save(os.path.join(EXPERIMENT_RESULT_PATH, "gcn_loss_history.npy"), loss_history)
        np.save(os.path.join(EXPERIMENT_RESULT_PATH, "gcn_acc_history.npy"), acc_history)
    else:
        logging.info(f'Loading model from {os.path.join(EXPERIMENT_RESULT_PATH, "gcn_model.pth")}')
        model.load_state_dict(torch.load(os.path.join(EXPERIMENT_RESULT_PATH, "gcn_model.pth"), weights_only=True))
        optimizer.load_state_dict(torch.load(os.path.join(EXPERIMENT_RESULT_PATH, "gcn_optimizer.pth"), weights_only=True))
        loss_history = np.load(os.path.join(EXPERIMENT_RESULT_PATH, "gcn_loss_history.npy"))
        acc_history = np.load(os.path.join(EXPERIMENT_RESULT_PATH, "gcn_acc_history.npy"))
    ...
    plot_training_results(EPOCH, loss_history, acc_history, os.path.join(EXPERIMENT_RESULT_PATH, "gcn_training_loss.png"))
    visualize_cmap(cmap, ground_truth, os.path.join(EXPERIMENT_RESULT_PATH, "gcn_visualize_cmap.png"))
    ...
    output_path = os.path.join(EXPERIMENT_RESULT_PATH, "gcn_embeddings.png")
    ...
    output_path = os.path.join(EXPERIMENT_RESULT_PATH, "gcn_embedding_graph.png")
